    ...
    >
      <div className="flex gap-4">
        <div className="shrink-0 flex flex-col items-center">
          <div
            className={`flex flex-col items-center justify-center w-14 h-9 rounded-lg border text-center ${ratingColor}`}
          >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col gap-0.5">
              <h2 className="text-sm font-bold text-gray-900 dark:text-gray-100 leading-snug tracking-tight hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                <Link href={`/tutorials/${id}`}>{title}</Link>
              </h2>
              {linkedQuestions.length > 0 && (
                <p className="text-[11px] text-gray-500 dark:text-gray-400">
                  Made for:{" "}
                  <span className="italic font-medium">
                    {linkedQuestions.map((q) => q.title).join(", ")}
                  </span>
                </p>
              )}
            </div>
