...
export default function QuestionDetailPage() {
  ...
  // ── Handlers (Prepend/Append for Optimistic feel) ──────────────────────────
  const handleAnswerPosted = useCallback((raw: any) => {
    // API returns { answer: {...} } or just {...}
    const newAnswer = raw.answer || raw;
    setAnswers((prev) => [newAnswer, ...prev]);
    showToast({ type: "success", title: "Success", message: "Answer posted!" });
  }, [showToast]);
  ...
  const handleAnswerReply = (answerId: number) => async (content: string, parentCommentId?: string) => {
    const res = await fetch(`/api/questions/${questionId}/answers/${answerId}/comments`, {
      method: "POST",
      headers: authHeaders(),
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      ...
      // Check both question-level and answer-level comments
      setComments((prev) => prev.filter((c) => String(c.comment_id) !== commentId));
      setAnswers((prev) => prev.map(a => ({
        ...a,
        comments: (a.comments ?? []).filter(c => String(c.comment_id) !== commentId)
      })));

      showToast({ type: "success", title: "Deleted", message: "Comment removed." });
    } catch (err: any) {
    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <Link href="/questions" className="text-primary-700 hover:underline">Back to List</Link>
        </div>
      </div>
    </div>
  );
        ...
        <main className="flex-1 py-6 px-4 sm:px-8 overflow-y-auto">
          <div className="max-w-4xl mx-auto flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => router.back()} className="flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-primary-700 transition-colors group">
              ...
              <div className="flex flex-col gap-5">
                ...
                {answers.length === 0 && (
                  <div className="border border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-10 text-center flex flex-col items-center gap-2">
                    ...
                    <p className="text-gray-400 italic text-sm">No answers yet. Be the first to help!</p>
                  </div>
                )}
              </div>
            </div>
